...
public class XiangQingModel {
    public void getData(String id,final XiangQingModelCallBack callBack){
        long timestamp = new Date().getTime() / 1000;//时间戳
        Map<String, String> map = new HashMap<>();
        map.put("timestamp", String.valueOf(timestamp));
        System.out.println("XiangQing:" + timestamp);
        map.put("id", id);

        try {
          String  sign = SignUtils.getSignature(map, Api.PRIVATE_KEY);

            System.out.println("XiangQing:" + sign);
            RetrofitUtil.api.postWenZh(id,sign,String.valueOf(timestamp))
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribe(new Consumer<ShouYe_XiangQingBean>() {
                        @Override
                        public void accept(ShouYe_XiangQingBean bean) throws Exception {
    ...
    public interface XiangQingModelCallBack {
